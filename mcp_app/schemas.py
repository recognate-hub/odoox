...
class CreateLeadInput(BaseModel):
    ...
    email: EmailStr | None = Field(
        None, description="A valid email address for the lead."
    )
    ...
    description: str | None = Field(
        None, max_length=1000, description="Additional notes or description."
    )
...
class ScheduleMeetingInput(BaseModel):
    ...
    notes: str | None = Field(
        "", max_length=2000, description="Meeting notes or agenda."
    )
...
class LogActivityInput(BaseModel):
    res_model: str = Field(
        ...,
        description="The Odoo model to attach the note to (e.g. 'crm.lead' or 'res.partner').",
    )
    res_id: int = Field(..., description="The ID of the record.")
    summary: str = Field(
        ..., max_length=1000, description="The content of the note or activity summary."
    )


class CreateInvoiceInput(BaseModel):
    partner_id: int = Field(
        ..., description="The ID of the customer (partner) to invoice."
    )
    amount: float = Field(
        ..., gt=0, description="The total amount for the invoice line item."
    )
    description: str = Field(
        "Consulting Services",
        max_length=500,
        description="The description for the invoice line item.",
    )
...
class SendEmailInput(BaseModel):
    ...
    subject: str = Field(
        ..., max_length=200, description="The subject line of the email."
    )
...
class CreateContactInput(BaseModel):
    name: str = Field(
        ..., max_length=150, description="The full name of the contact or company."
    )
    email: EmailStr | None = Field(
        None, description="The email address of the contact."
    )
    phone: str | None = Field(
        None,
        max_length=20,
    ...
    is_company: bool = Field(
        False,
        description="Set to true if this contact represents a company rather than an individual.",
    )


class CreateProductInput(BaseModel):
    name: str = Field(
        ..., max_length=150, description="The name of the product or service."
    )
    list_price: float = Field(..., ge=0, description="The sale price of the product.")
    default_code: str | None = Field(
        None,
        max_length=50,
        description="The internal reference or SKU for the product.",
    )
    product_type: str = Field(
        "service",
        description="Product type. Usually 'consu' (Consumable), 'service' (Service), or 'product' (Storable Product).",
    )
...
class QuoteLineInput(BaseModel):
    ...
    price_unit: float | None = Field(
        None,
        ge=0,
        description="The unit price. If omitted, Odoo will use the product's default list price.",
    )


class CreateQuoteInput(BaseModel):
    partner_id: int = Field(
        ..., description="The ID of the customer (partner) to create the quote for."
    )
    order_lines: list[QuoteLineInput] = Field(
        ...,
        min_length=1,
        description="List of products and quantities to include in the quote.",
    )
...
class SearchReadInput(BaseModel):
    ...
    model: str = Field(
        ..., description="The Odoo model name (e.g. 'hr.employee', 'project.task')."
    )
    domain: list[list[Any]] | None = Field(
        None, description="The search domain to filter records."
    )
    fields: list[str] | None = Field(
        None,
        description="List of specific fields to return. If omitted, all fields are returned.",
    )
    limit: int = Field(
        50, ge=1, le=200, description="The maximum number of records to return."
    )
    offset: int = Field(
...
class CreateRecordInput(BaseModel):
    ...
    data: dict[str, Any] = Field(
        ..., description="Dictionary of fields and values to create the record."
    )
...
class UpdateRecordInput(BaseModel):
    ...
    data: dict[str, Any] = Field(
        ..., description="Dictionary of fields and values to update."
    )
...
class BatchCreateRecordInput(BaseModel):
    ...
    records: list[dict[str, Any]] = Field(
        ...,
        description="List of dictionaries of fields and values to create multiple records.",
    )
...
class BatchUpdateRecordInput(BaseModel):
    ...
    record_ids: list[int] = Field(
        ..., min_length=1, description="The IDs of the records to update."
    )
    data: dict[str, Any] = Field(
        ...,
        description="Dictionary of fields and values to apply to all given record IDs.",
    )
...
class ReadGroupInput(BaseModel):
    ...
    fields: list[str] = Field(
        ...,
        description="List of fields to fetch/aggregate. Must include the groupby fields.",
    )
...
class ArchiveRecordInput(BaseModel):
    ...
    model: str = Field(..., description="The Odoo model name.")
    record_id: int = Field(
        ..., gt=0, description="The ID of the record to archive/unarchive."
    )
    archive: bool = Field(
        True,
        description="True to archive (active=False), False to unarchive (active=True).",
    )
...
class ReadAttachmentInput(BaseModel):
    attachment_id: int = Field(
        ..., gt=0, description="The ID of the attachment to read."
    )
...
class ExecuteMethodInput(BaseModel):
    model: str = Field(..., description="The Odoo model name.")
    method: str = Field(
        ..., description="The method to call on the model (e.g. 'action_confirm')."
    )
...
class PostMessageInput(BaseModel):
    ...
    message_type: str = Field(
        "comment", description="The type of message (e.g., 'comment', 'notification')."
    )
...
class CreateChannelInput(BaseModel):
    ...
    channel_type: str = Field(
        "channel", description="The type of the channel ('channel', 'chat', 'group')."
    )
...
class CreatePurchaseOrderInput(BaseModel):
    ...
    order_lines: list[PurchaseOrderLineInput] = Field(
        ..., min_length=1, description="List of products and quantities to purchase."
    )
...
class CreateQualityAlertInput(BaseModel):
    ...
    product_id: int = Field(
        ..., description="The ID of the product related to the alert."
    )
    team_id: int | None = Field(None, description="The ID of the quality team.")
    priority: str | None = Field(
        "0", description="Priority of the alert ('0', '1', '2', '3')."
    )
...
class UpdateMeetingInput(BaseModel):
    meeting_id: int = Field(
        ..., gt=0, description="The ID of the calendar event to update."
    )
    data: dict[str, Any] = Field(
        ..., description="Fields to update (e.g., name, start, stop, location)."
    )


class DeleteMeetingInput(BaseModel):
    meeting_id: int = Field(
        ..., gt=0, description="The ID of the calendar event to delete."
    )
...
class UpdateQualityAlertInput(BaseModel):
    alert_id: int = Field(
        ..., gt=0, description="The ID of the quality alert to update."
    )
    data: dict[str, Any] = Field(
        ..., description="Fields to update (e.g., stage_id, priority, description)."
    )
...
class UpdateManufacturingOrderInput(BaseModel):
    mo_id: int = Field(
        ..., gt=0, description="The ID of the manufacturing order to update."
    )
    data: dict[str, Any] = Field(
        ..., description="Fields to update (e.g., product_qty, date_start)."
    )


class ConfirmManufacturingOrderInput(BaseModel):
    mo_id: int = Field(
        ..., gt=0, description="The ID of the manufacturing order to confirm."
    )
...
class ConfirmPurchaseOrderInput(BaseModel):
    po_id: int = Field(
        ..., gt=0, description="The ID of the purchase order to confirm."
    )
...
class PostInvoiceInput(BaseModel):
    invoice_id: int = Field(
        ..., gt=0, description="The ID of the invoice to post/validate."
    )


class RegisterPaymentInput(BaseModel):
    invoice_id: int = Field(
        ..., gt=0, description="The ID of the invoice to register payment for."
    )
    amount: float = Field(..., gt=0, description="The payment amount.")
    journal_id: int = Field(
        ..., gt=0, description="The ID of the payment journal (bank/cash)."
    )
...
class CreateEcoInput(BaseModel):
    ...
    type_id: int = Field(
        ..., description="The ID of the ECO type (e.g. routing, BOM change)."
    )
    name: str = Field(
        ..., description="Name or title for the Engineering Change Order."
    )
...
class RescheduleWorkOrderInput(BaseModel):
    ...
    date_finished: str = Field(
        ..., description="New planned finished date (ISO string)."
    )
...
class LogWorkOrderTimeInput(BaseModel):
    ...
    loss_id: int | None = Field(
        None, description="ID of the productivity loss reason if this was downtime."
    )
...
class CreateSubcontractingPoInput(BaseModel):
    ...
    product_id: int = Field(
        ..., description="The ID of the finished product to receive."
    )
    product_qty: float = Field(..., gt=0, description="Quantity to subcontract.")
    price_unit: float | None = Field(
        None, description="Price per unit paid to subcontractor."
    )


class QuoteToCashInput(BaseModel):
    partner_id: int = Field(..., description="The ID of the customer (partner).")
    order_lines: list[QuoteLineInput] = Field(
        ...,
        min_length=1,
        description="List of products and quantities to include in the quote.",
...
class BatchImportCsvInput(BaseModel):
    model_config = {"protected_namespaces": ()}
    model: str = Field(..., description="The Odoo model name.")
    csv_data: str = Field(
        ..., description="The raw CSV data to import (must include header row)."
    )
