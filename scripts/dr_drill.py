...
def run_drill():
    ...
    try:
        ...
        try:
            ...
            print(f"Backup successfully uploaded to Supabase Storage bucket 'backups': {file_name}")
        except Exception as upload_err:
            if "Bucket not found" in str(upload_err):
                print("ERROR: Supabase Storage bucket 'backups' not found.")
                print("Please create a bucket named 'backups' in your Supabase dashboard.")
    ...
    if record_count == 0:
        print("Warning: Table is empty. The drill will proceed, but there is no data to wipe/restore.")
    ...
    try:
        if record_count > 0:
            for record in original_data:
                supabase.table(TABLE_NAME).delete().eq("user_id", record["user_id"]).execute()
    ...
    try:
        if record_count > 0:
            ...
            restore_response = supabase.table(TABLE_NAME).insert(restored_data).execute()
