...
class DataGovernanceService:
    ...
    def export_tenant_data(self, user_id: str) -> dict[str, Any]:
        ...
        workspace_response = (
            self.supabase.table("user_workspaces")
            .select("*")
            .eq("user_id", user_id)
            .execute()
        )
        ...
        return {
            ...
            "middleware_data": {"workspaces": workspace_response.data},
        }
    ...
    def delete_tenant_data(self, user_id: str) -> dict[str, Any]:
        ...
        if not verification_passed:
            ...
            raise RuntimeError(
                "Data deletion verification failed. user_id still found in logs."
            )

        logger.info(
            "Tenant deletion workflow completed",
