    ...
    def _get_tenant_usage(self, tenant_id: str) -> int:
        ...
        if redis_client:
            try:
                ...
                logger.warning(
                    f"Redis get failed for FinOps, falling back to in-memory: {e}"
                )
                if self.strict_redis:
                    raise RuntimeError(
                        "Redis is required for FinOps in production but is unavailable."
                    ) from e
        elif self.strict_redis:
            raise RuntimeError(
                "Redis is required for FinOps in production but is not configured."
            )

        # In-memory fallback
        if (
            tenant_id not in self._usage_store
            or self.today not in self._usage_store[tenant_id]
        ):
    ...
    def record_invocation(self, tenant_id: str, tool_name: str) -> None:
        ...
        if current_usage >= self.daily_budget_limit:
            ...
            raise FinOpsBudgetExceededException(
                f"Tenant {tenant_id} has exceeded their daily API budget of {self.daily_budget_limit} requests."
            )
        ...
        if redis_client:
            try:
                ...
                logger.warning(
                    f"Redis incr failed for FinOps, falling back to in-memory: {e}"
                )
                if self.strict_redis:
                    raise RuntimeError(
                        "Redis is required for FinOps in production but is unavailable."
