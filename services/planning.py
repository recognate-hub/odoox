...
class PlanningService:
    ...
    def get_production_planning_data(self, months: int = 4) -> list[dict[str, Any]]:
        ...
        so_records = self.odoo.search_read_records("sale.order", so_domain, ["id"], limit=5000)
        ...
        if so_ids:
            sol_records = self.odoo.search_read_records(
                "sale.order.line", sol_domain, ["product_id", "product_uom_qty"], limit=10000
            )
        ...
        pending_lines = self.odoo.search_read_records(
            "sale.order.line", pending_domain, ["product_id", "product_uom_qty", "qty_delivered", "order_partner_id", "order_id"], limit=10000
        )
        ...
        workorders = self.odoo.search_read_records(
            "mrp.workorder", wip_domain, ["product_id", "workcenter_id", "qty_production"], limit=10000
        )
        ...
        for line in pending_lines:
            ...
            if pid:
                ...
                if pending > 0:
                    ...
                    customer_name = get_product_name(partner_id) if partner_id else "Unknown"
