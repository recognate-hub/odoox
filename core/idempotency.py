...
class IdempotencyCache:
    ...
    def check_or_execute(
        tenant_db: str, tool_name: str, payload: Any, execute_fn: Callable[[], Any]
    ) -> Any:
        now = time.time()
        ...
        expired_keys = [
            k
            for k, (res, ts) in _idempotency_state.items()
        ...
        if idem_key in _idempotency_state:
            cached_result, _ts = _idempotency_state[idem_key]
            logger.info(
                "Idempotency cache hit. Returning cached result to prevent duplicate write.",
                tool=tool_name,
                tenant=tenant_db,
            )
