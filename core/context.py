...
def get_workspace_credentials(
    token: str, workspace_id: str | None = None, force_refresh: bool = False
) -> WorkspaceContext:
    ...
    if len(_credentials_cache) > MAX_CACHE_SIZE:
        expired_keys = [
            k for k, (_, ts) in _credentials_cache.items() if now - ts >= CACHE_TTL_SEC
        ]
    ...
    if token.startswith("odx_"):
        try:
            ...
            if cached_revocation != "0":  # Not cached yet — check Supabase
                ...
                try:
                    revoked_resp = (
                        supabase_admin.table("revoked_api_keys")
                        .select("id")
                        .eq("api_key", token)
                        .limit(1)
                        .execute()
                    ...
                    if revoked_resp.data:
                        set_cached_value(
                            revoked_cache_key, "1", ttl=86400
                        )  # Cache for 24h
                        logger.warning("Revoked API Key attempt", token=token[:20])
                        raise HTTPException(
                            status_code=401, detail="API Key has been revoked"
                        )
                    else:
                        set_cached_value(
                            revoked_cache_key, "0", ttl=300
                        )  # Cache "not revoked" for 5 min
                except Exception as db_err:
                    if (
                        "PGRST205" in str(db_err)
                        or "does not exist" in str(db_err).lower()
                    ):
                        logger.warning(
                            "revoked_api_keys table missing, skipping revocation check"
                        )
    ...
    try:
        ...
        logger.info(
            "Fetched fresh credentials from database",
            user_id=user_id,
        ...
        if any(
            phrase in error_str
            for phrase in ("token has expired", "jwt expired", "token expired")
        ):
            logger.warning(
        ...
        raise HTTPException(
            status_code=401, detail=f"Could not fetch workspace credentials: {e!s}"
        )
