...
async def post_verify_otp(
    request: Request, email: str = Form(...), token: str = Form(...)
):
...
def get_workspace(request: Request, token: str = Depends(get_user_token)):
    ...
    workspace_response = (
        supabase.table("user_workspaces").select("*").eq("user_id", user_id).execute()
    )
    # type: ignore
    workspace = (
        workspace_response.data[0]
        if workspace_response.data and isinstance(workspace_response.data, list)
        else None
    )
...
def api_save_config(
    ...
    if not workspace_id and len(existing_data) >= max_workspaces:
        return {
            "status": "error",
            "message": f"Plan limit reached. {plan_type.capitalize()} plan allows {max_workspaces} workspace(s).",
        }
    ...
    if workspace_id:
        target_workspace = next(
            (w for w in existing_data if str(w.get("id")) == str(workspace_id)), None
        )
    ...
    try:
        if target_workspace:
            supabase.table("user_workspaces").update(payload).eq(
                "id", workspace_id
            ).execute()
        else:
...
async def api_delete_config(request: Request, token: str = Depends(get_user_token)):
    ...
    try:
        supabase.table("user_workspaces").delete().eq("id", workspace_id).eq(
            "user_id", user_id
        ).execute()
...
def revoke_api_key(
    request: Request, api_key: str = Form(...), token: str = Depends(get_user_token)
):
    """
    ...
    try:
        try:
            (
                supabase.table("revoked_api_keys")
                .insert(
                    {
                        "api_key": api_key,
            ...
            logger.warning(
                f"Failed to insert revoked key into DB (table might be missing), relying on Redis cache: {db_err}"
            )
...
def save_config(
    ...
    return api_save_config(
        request, token, odoo_url, odoo_db, odoo_username, odoo_password
    )
