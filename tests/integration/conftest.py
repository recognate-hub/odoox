...
def mock_odoo_connector():
    ...
    connector.get_leads.return_value = [
        OdooLead(
            id=1,
            name="Alpha Corp Deal",
            email_from="alice@alpha.com",
            phone="555-0101",
            partner_id=[10, "Alpha Corp"],
    ...
    connector.search_contacts.return_value = [
        ...
        OdooContact(
            id=11, name="Alice Smith", email="alice@alpha.com", is_company=False
        ),
    ]
...
def mock_claude_service():
    ...
    service.analyze_lead.return_value = (
        "AI Lead Analysis: Strong potential for enterprise deal."
    )
    service.summarize_meeting.return_value = "Meeting Summary: Key decisions were made."
    service.generate_email.return_value = (
        "Dear Alice, I wanted to follow up on our conversation..."
    )
    service.customer_summary.return_value = (
        "Customer 360: Alpha Corp is a key account with high LTV."
    )
    service.sales_forecast.return_value = (
        "Forecast: Pipeline is healthy with $23K expected revenue."
    )
    ...
    service.conversation_summary.return_value = (
        "Thread Summary: Last 3 exchanges discussed pricing."
    )
    return service
