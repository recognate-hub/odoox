...
def test_authenticate_success(mock_redis, mock_token, mock_workspace):
    with patch.object(
        XmlRpcOdooConnector, "_get_workspace", return_value=mock_workspace
    ), patch.object(XmlRpcOdooConnector, "_get_common") as mock_get_common:
...
def test_public_methods(mock_exec, connector):
    ...
    with patch(
        "odoo.xmlrpc.IdempotencyCache.check_or_execute",
        side_effect=lambda db, k, d, f: f(),
    ), patch.object(
        XmlRpcOdooConnector, "_get_workspace", return_value=MagicMock()
    ):
