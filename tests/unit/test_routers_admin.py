...
def test_post_verify_otp_success(mock_get_supabase):
    ...
    response = client.post(
        "/login/verify", data={"email": "t@t.com", "token": "123456"}
    )
    assert response.status_code == 200
...
def test_api_save_config_success(mock_get_supabase):
    ...
    data = {"odoo_url": "u", "odoo_db": "d", "odoo_username": "n", "odoo_password": "p"}
    response = client.post(
        "/api/workspace/save", data=data, cookies={"access_token": "acc"}
    )
    assert response.status_code == 200
    assert response.json()["status"] == "success"
