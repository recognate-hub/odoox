...
def test_is_model_allowed_admin(policy_file):
    PolicyEngine.load_policies(policy_file)
    assert PolicyEngine.is_model_allowed("Admin", "search_read_records", "any.model") is True

def test_is_model_allowed_sales(policy_file):
    PolicyEngine.load_policies(policy_file)
    assert PolicyEngine.is_model_allowed("Sales", "search_read_records", "crm.lead") is True
    assert PolicyEngine.is_model_allowed("Sales", "search_read_records", "res.partner") is True
    assert PolicyEngine.is_model_allowed("Sales", "search_read_records", "hr.employee") is False

def test_is_model_allowed_unknown_role(policy_file):
    PolicyEngine.load_policies(policy_file)
    assert PolicyEngine.is_model_allowed("Hacker", "search_read_records", "crm.lead") is False
