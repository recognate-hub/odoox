...
def test_authorize_redirect_login():
    response = client.get(
        "/oauth/authorize?client_id=1&redirect_uri=http://l", follow_redirects=False
    )
...
def test_authorize_success():
    response = client.get(
        "/oauth/authorize?client_id=1&redirect_uri=http://l",
...
def test_authorize_success_with_state():
    response = client.get(
        "/oauth/authorize?client_id=1&redirect_uri=http://l&state=xyz",
        cookies={"access_token": "acc"},
        follow_redirects=False,
    )
    assert response.status_code == 303
...
def test_token_auth_code_success(mock_get_workspace):
    ...
    payload = {"access_token": "acc", "refresh_token": "ref", "exp": time.time() + 300}
...
def test_token_refresh_success(mock_get_settings, mock_create_client):
    ...
    response = client.post(
        "/oauth/token", data={"grant_type": "refresh_token", "refresh_token": enc_ref}
    )
    assert response.status_code == 200
