...
def test_secure_tool_rbac_rejection(mock_is_allowed, mock_get_context):
    ...
    with pytest.raises(
        ...
        match="Role Sales does not have permission to execute test_action",
    ):
        dummy_tool()
...
def test_secure_tool_model_rbac_rejection(
    mock_is_model_allowed, mock_is_allowed, mock_get_context
):
    mock_get_context.return_value = UserContext(user_id="user1", role="Sales")
    ...
    with pytest.raises(
        PermissionDeniedError,
        match="Role Sales does not have permission to access model restricted.model",
    ):
...
@patch("mcp_app.security.PolicyEngine.is_allowed")
@patch("mcp_app.security.PolicyEngine.is_model_allowed")
def test_secure_tool_model_rbac_success(
    mock_is_model_allowed, mock_is_allowed, mock_get_context
):
    mock_get_context.return_value = UserContext(user_id="user1", role="Sales")
    mock_is_allowed.return_value = True
