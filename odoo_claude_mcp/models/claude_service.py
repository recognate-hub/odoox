...
class ClaudeService(models.AbstractModel):
    ...
    def _get_client(self):
        if not anthropic:
            raise UserError("The 'anthropic' python package is not installed. Please run 'pip install anthropic'.")
            
        api_key = self.env['ir.config_parameter'].sudo().get_param('odoo_claude_mcp.claude_api_key')
        if not api_key:
            raise UserError("Anthropic API key is not configured. Please set it in Settings -> General Settings.")
            
        return anthropic.Anthropic(api_key=api_key)

    @api.model
    def _get_model_name(self):
        return self.env['ir.config_parameter'].sudo().get_param('odoo_claude_mcp.claude_model_name', 'claude-3-5-sonnet-20240620')

    @api.model
    def _get_temperature(self):
        temp = self.env['ir.config_parameter'].sudo().get_param('odoo_claude_mcp.claude_temperature', 0.0)
        return float(temp)
